# Foundational OHLCV Quant Research Sources

Primary papers, executable notebooks, and scientific references for any asset and bar interval

Verified online: 2026-07-13

This is a market-agnostic source guide for starting with timestamped open, high, low, close, and volume data. It applies to equities, futures, FX, crypto, commodities, rates, and other regularly sampled instruments; the sampling interval can be seconds, minutes, hours, or days. The sources cover return predictability, volatility, regimes, machine learning, validation, and execution without assuming that one asset or horizon is inherently forecastable.

The list prioritizes primary papers, university material, official scientific-library documentation, and executable Jupyter notebooks. It excludes influencer trading content, indicator-list articles, and unsupported prediction demos. It routes you to established methods; it is not a prewritten strategy or a claim that OHLCV alone contains tradable edge.

## Legend

| Label | Meaning |
|---|---|
| **Direct** | Can be applied to one OHLCV series after replacing the example data and defining the forecast horizon. |
| **Adapt** | The method transfers, but the source uses another market, sampling interval, or research setting. |
| **More data** | Requires another asset or venue, derivatives fields, underlying trades, quotes, or order-book data. |
...
## Source map: start from the research question

This table is an index into the material below, not a homemade recipe. Each row points to a distinct research school and its strongest starting references.

| Research question supported by OHLCV | Start with | Natural target or object |
|---|---|---|
| Is there serial dependence in returns or ranges? | [Lo–MacKinlay variance ratio](https://rodneywhitecenter.wharton.upenn.edu/wp-content/uploads/2014/04/8705.pdf), [statsmodels time-series tools](https://www.statsmodels.org/stable/tsa.html) | Returns, signed moves, ranges, autocorrelation, variance ratios |
| Is volatility more predictable than direction? | [ARCH/GARCH documentation](https://arch.readthedocs.io/en/latest/univariate/univariate_volatility_modeling.html), [Corsi HAR-RV](https://statmath.wu.ac.at/~hauser/LVs/FinEtricsQF/References/Corsi2009JFinEtrics_LMmodelRealizedVola.pdf) | Future realized variance, absolute return, high-low range, tail quantiles |
| Does the data change state or break structurally? | [Hamilton regime switching](https://www.ssc.wisc.edu/~bhansen/718/Hamilton1989.pdf), [Ruptures](https://github.com/deepcharles/ruptures) | Latent regimes, change points, rolling parameter instability |
| Can nonlinear models improve a simple baseline? | [Scikit-learn lagged-feature notebook](https://scikit-learn.org/stable/auto_examples/applications/plot_time_series_lagged_features.html), [official PyTorch sequence example](https://github.com/pytorch/examples/tree/main/time_sequence_prediction) | Forward return, direction/event labels, range, volatility, quantiles |
| Did the apparent winner survive model search? | [White Reality Check](https://www.ssc.wisc.edu/~bhansen/718/White2000.pdf), [ARCH SPA notebook](https://arch.readthedocs.io/en/stable/multiple-comparison/multiple-comparison_examples.html) | Candidate-model loss series, trial-adjusted performance |
| Is forecast accuracy still useful after trading frictions? | [VectorBT](https://vectorbt.dev/), [Almgren–Chriss](https://www.smallake.kr/wp-content/uploads/2016/03/optliq.pdf) | Net returns, turnover, slippage, impact, capacity |
| What requires data beyond one OHLCV series? | [Engle–Granger](https://users.ssc.wisc.edu/~behansen/718/EngleGranger1987.pdf), [DeepLOB](https://arxiv.org/pdf/1808.03668), [perpetual futures pricing](https://finance.wharton.upenn.edu/~jermann/AHJ-main-10.pdf) | Relative value, basis/carry, order flow, book imbalance |
...
| [PyTorch official time-sequence prediction example](https://github.com/pytorch/examples/tree/main/time_sequence_prediction) | **Direct · minimal code** | A deliberately small LSTM example using two `LSTMCell` layers. It is a clean way to learn tensor shapes, recurrent state, training loops, and multi-step prediction mechanics before touching a large forecasting framework. Predicting its toy sine wave is not evidence of financial predictability. |
...
| [NeuralForecast robust forecasting notebook](https://nixtlaverse.nixtla.io/neuralforecast/docs/tutorials/robust_forecasting.html) | **Direct · Colab notebook** | Huber and multi-quantile losses plus dropout for noisy and outlier-heavy time series. This is a concrete alternative to automatically deleting large market moves as “bad data.” |
...
| [Statsmodels seasonal/state-space notebook](https://www.statsmodels.org/stable/examples/notebooks/generated/statespace_seasonal.html) | **Direct · notebook** | Shows how to represent more than one periodic component. Relevant cycles depend on the market and bar interval: session openings, time of day, day of week, contract events, auctions, or settlement schedules. |
...
| Resource | Fit | Concrete use |
|---|---|---|
| [Qlib workflow notebook](https://github.com/microsoft/qlib/blob/main/examples/workflow_by_code.ipynb) | **Adapt · Jupyter notebook** | End-to-end data/model/record/backtest/report wiring. Best viewed as an example of reproducible experiment structure; its default Alpha158 equity setup must be replaced with features, targets, and costs appropriate to the dataset. |
...
| [DeepLOB paper repository and Jupyter notebooks](https://github.com/zcakhaa/DeepLOB-Deep-Convolutional-Neural-Networks-for-Limit-Order-Books) | **More data · PyTorch/TensorFlow notebooks** | The canonical CNN+LSTM order-book model implementation. It is deliberately listed as a boundary: it cannot be reproduced honestly from aggregated OHLCV bars and becomes relevant only after acquiring synchronized depth-of-book data. |

### Sampling, labeling, costs, and backtesting

| Resource | Fit | Concrete use |
|---|---|---|
| [Mlfin.py financial data structures](https://mlfinpy.readthedocs.io/en/latest/FinancialDataStructure.html) | **More data · code-first** | Time, tick, volume, dollar, imbalance, and run bars. True tick, dollar, imbalance, and run bars require underlying trades; aggregated OHLCV cannot reconstruct event order inside each bar. |
| [Mlfin.py CUSUM filtering](https://mlfinpy.readthedocs.io/en/latest/Filtering.html) | **Direct · code-first** | Event sampling based on accumulated deviations instead of labeling every bar. This creates a different dataset rather than another indicator on the same fixed clock. |
...
5. [Liu and Tsyvinski — *Risks and Returns of Cryptocurrency*](https://www.nber.org/system/files/working_papers/w24877/w24877.pdf) — **Paper · Asset-class example.** Finds cryptocurrency-specific time-series momentum and investor-attention predictors at daily and weekly frequencies. Use it when the dataset is crypto; it is not a universal OHLCV result.
...
3. [Andersen, Bollerslev, Diebold, and Labys — *Modeling and Forecasting Realized Volatility*](https://www.nber.org/system/files/working_papers/w8160/w8160.pdf) — **Paper · Direct when intraday bars are available.** Shows how intraday returns construct ex-post realized volatility and how that measure can be modeled and forecast. Fine-grained bars can build higher-horizon realized-variance targets, subject to market microstructure noise.
...
7. [Corwin and Schultz — *A Simple Way to Estimate Bid-Ask Spreads from Daily High and Low Prices*](https://users.nber.org/~confer/2009/mms09/Corwin_Schultz.pdf) — **Paper · Adapt.** Infers a spread component from high-low behavior over adjacent intervals. It was designed for daily equity data; elsewhere it is best treated as a candidate liquidity proxy and validated against actual quote spreads when available.
...
2. [Gatev, Goetzmann, and Rouwenhorst — *Pairs Trading: Performance of a Relative-Value Arbitrage Rule*](https://stat.wharton.upenn.edu/~steele/Courses/434/434Context/PairsTrading/PairsTradingGGR.pdf) — **Paper · More data.** A simple distance-based formation and trading rule with bootstrap analysis and transaction costs. Adaptable to related assets, venues, share classes, calendar spreads, spot/futures pairs, or factor-neutral residuals.

3. [Avellaneda and Lee — *Statistical Arbitrage in the U.S. Equities Market*](https://traders.berkeley.edu/papers/Statistical%20arbitrage%20in%20the%20US%20equities%20market.pdf) — **Paper · More data.** Uses PCA or sector-factor regression to remove common movement, then models residuals as mean-reverting processes. This is a core reference for discovering hedged residual alpha across a universe rather than predicting every instrument outright.

4. [Statsmodels VAR/VECM documentation](https://www.statsmodels.org/devel/vector_ar.html) — **Code-first reference · More data.** Vector autoregression, Granger causality, impulse responses, cointegration-rank tests, and vector error-correction models for multiple aligned price or explanatory series.

### 5. Optional derivative and asset-class extensions

These sources do not apply to generic OHLCV alone. Use them only when the instrument and accompanying fields match the market structure described.

1. [Ackerer, Hugonnier, and Jermann — *Perpetual Futures Pricing*](https://finance.wharton.upenn.edu/~jermann/AHJ-main-10.pdf) — **Paper · More data · crypto derivatives.** Derives no-arbitrage pricing for linear, inverse, and quanto perpetual futures and formalizes the funding mechanism that anchors perpetuals to spot. Foundational before treating funding as a predictor or carry return.

2. [He, Manela, Ross, and von Wachter — *Fundamentals of Perpetual Futures*](https://arxiv.org/abs/2212.06888) — **Paper · More data · crypto derivatives.** Studies perpetual-contract mechanics and funding design. Useful for defining P&L correctly and separating directional return from funding and basis convergence.

3. [Schmeling, Schrimpf, and Todorov — *Crypto Carry*](https://www.bis.org/publ/work1087.pdf) — **Institutional paper · More data · crypto derivatives.** Empirical study of spot, futures basis, volume, open interest, and options characteristics across exchanges. It opens a carry/basis school of research that OHLCV alone cannot represent.

4. [Liu, Tsyvinski, and Wu — *Common Risk Factors in Cryptocurrency*](https://www.nber.org/system/files/working_papers/w25882/w25882.pdf) — **Paper · More assets · crypto cross-section.** Builds market, size, momentum, volume, and volatility factors across a large cryptocurrency universe. Relevant only after research expands from one series into cross-sectional ranking and hedged factor portfolios.
...
## Quick source index by practical question

These combinations are links to existing material, not a new research framework.

| Question | Sources that directly address it |
|---|---|
| “Are returns serially predictable at any aggregation horizon?” | [Lo–MacKinlay](https://rodneywhitecenter.wharton.upenn.edu/wp-content/uploads/2014/04/8705.pdf), [Quantopian AR notebook](https://github.com/quantopian/research_public/blob/master/notebooks/lectures/Autocorrelation_and_AR_Models/notebook.ipynb), [statsmodels TSA](https://www.statsmodels.org/stable/tsa.html) |
| “Can I build a simple volatility predictor from these bars?” | [Andersen et al.](https://www.nber.org/system/files/working_papers/w8160/w8160.pdf), [Corsi HAR-RV](https://statmath.wu.ac.at/~hauser/LVs/FinEtricsQF/References/Corsi2009JFinEtrics_LMmodelRealizedVola.pdf), [HAR-RV notebook](https://github.com/deep-hedger-Peng/HAR-RV), [ARCH forecasting](https://arch.readthedocs.io/en/latest/univariate/univariate_volatility_forecasting.html) |
...
| “What becomes possible after adding related contracts, assets, venues, or derivatives fields?” | [Engle–Granger](https://users.ssc.wisc.edu/~behansen/718/EngleGranger1987.pdf), [Pairs Trading](https://stat.wharton.upenn.edu/~steele/Courses/434/434Context/PairsTrading/PairsTradingGGR.pdf), [Perpetual Futures Pricing](https://finance.wharton.upenn.edu/~jermann/AHJ-main-10.pdf), [Crypto Carry](https://www.bis.org/publ/work1087.pdf) |
...
## What OHLCV can and cannot support

- OHLCV supports return, range, volatility, volume, seasonality, change-point, regime, and bar-level liquidity-proxy research at the recorded interval and coarser aggregations.
- OHLCV alone does **not** reveal trade direction, bid/ask quotes, queue position, order-book imbalance, open interest, funding, spot/futures basis, liquidation events, or the exact path inside a bar.
...
- A fine-grained OHLCV file can be aggregated to coarser bars; a coarse file cannot recover the missing intrabar sequence. True tick, volume, dollar, imbalance, and run bars require the underlying trades.
- Papers from different markets or horizons are included for their definitions and testing methods. Their reported parameter values are not direct recommendations for another asset or interval.
